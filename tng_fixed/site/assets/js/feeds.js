window.TechNetGameFeeds = {
  ...
  resolveApiUrl(url = '') {
    if (window.TechNetGameRuntime?.withApiBase) {
      return window.TechNetGameRuntime.withApiBase(url);
    }
    return url;
  },

  async fetchJson(url) {
    const response = await fetch(this.resolveApiUrl(url), { headers: { Accept: 'application/json' } });
