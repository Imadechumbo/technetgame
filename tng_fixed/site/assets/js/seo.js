...
(() => {
  ...
  async function applyHomeHeroSeo() {
    ...
    try {
      ...
      const homeApi = window.TechNetGameRuntime?.withApiBase ? window.TechNetGameRuntime.withApiBase('/api/news/home') : '/api/news/home';
      const res = await fetch(homeApi, { signal: controller.signal, headers: { Accept: 'application/json' } });
