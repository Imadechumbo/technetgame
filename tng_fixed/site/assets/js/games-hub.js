document.addEventListener('DOMContentLoaded', () => {
  ...
  async function hydrateGameCover(img) {
    ...
    try {
      const response = await fetch(`/api/media/game-image?title=${encodeURIComponent(title)}&fallback=${encodeURIComponent(fallbackSrc)}`);
  ...
  async function hydrateCreatorAvatar(img) {
    ...
    try {
      const response = await fetch(`/api/media/creator-avatar?name=${encodeURIComponent(name)}&channelUrl=${encodeURIComponent(channelUrl)}&fallback=${encodeURIComponent(fallbackSrc)}`);
